...
$GLOBALS['warteZeit'] = [];
$GLOBALS['endhaltstellen'] = ["T17" => "Werdhölzli", "T13" => "Frankental", "T5" => "Laubegg"];
...
class Haltestelle
{
	...
	function wartezeitVorbereitung($tramnr)
	{
		...
		$arr = [
			"T17" => [
				...
				"1409", "1410", "1417", "1424", "1431", "1438", "1445", "1452", "1530", "1625"
			],

			"T5" => [
				"0840", "1040", "1402", "2227", "2234", "2241", "2248", "2255",
	...
	{
		$standardFormat = "<div class='row'><div class='tramnr col-xs-offset-2 col-xs-1'>" . '%s' . "</div>" .
			"<div class='endhaltestelle col-xs-6'>" . '%s' . "</div>" .
			"<div class='wartezeit col-xs-pull-2 col-xs-3 '>" . '%d\'' . "</div></div>";

		$tramAnkunftFormat = "<div class='row'><div class='tramnr col-xs-offset-2 col-xs-1'>" . '%s' . "</div>" .
			"<div class='endhaltestelle col-xs-6'>" . '%s' . "</div>" .
			"<div class='wartezeit col-xs-pull-2 col-xs-3'><div class='bus-parent'><div class='icon-bus3'></div></div></div></div>";

		$trBD = sprintf($tramAnkunftFormat, $tramnr, $GLOBALS['endhaltstellen'][$tramnr], $GLOBALS['warteZeit'][$tramnr]);
		$trND = sprintf($standardFormat, $tramnr, $GLOBALS['endhaltstellen'][$tramnr], $GLOBALS['warteZeit'][$tramnr] / 60);
	...
	function sortit()
	{
		...
		$GLOBALS['titleData'] = array_merge_recursive($GLOBALS['warteZeit'], $GLOBALS['endhaltstellen']);
//			echo "<pre>";
//			print_r($GLOBALS['titleData']);
//			echo "</pre>";
	}
...
$haltestelle->wartezeitVorbereitung("T17");
$haltestelle->wartezeitVorbereitung("T5");
$haltestelle->wartezeitVorbereitung("T13");
$haltestelle->sortit();
...
<head>
	...
	// item werte abholen (array key/value) und abspeichern;
	
	$tramname = array_keys($GLOBALS['titleData'])[0];
	$endhaltstelle = array_values($GLOBALS['titleData'])[0][1];
	$waitingtime = array_values($GLOBALS['titleData'])[0][0] / 60;

	//format mit abfüllende variabeln
	
	$title = sprintf('%s - %s - %d\'', $tramname, $endhaltstelle, $waitingtime);
